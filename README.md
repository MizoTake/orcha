...
`orcha` はフェーズ制御自体は自動化しますが、実際に完走しやすいのは次の条件を満たす構成です。

- `impl/fix` を担当する agent が **CLI 経由で実ファイルを変更できる**こと
- `execution.verification.commands` に **実際の検証コマンド** が入っていること
- `blocked` タスクが出たら人手介入が必要になる前提で運用すること

特に `mode: "http"` の agent は返答テキスト中心になりやすく、`impl/fix` の完了条件である「Git差分あり」と噛み合わず停止しやすいです。  
そのため、初期設定は `local_llm.mode: "cli"` を推奨しています。
...
```powershell
./scripts/run.ps1 run
# 仕様書から goal/criteria/tasks を自動生成して開始
./scripts/run.ps1 run --spec requirements.md
```

`run` は `Decide` フェーズで完了判定し、未完了なら次サイクルへ進みます。
`--spec` 未指定でも、タスク表が未初期化なら `goal.md` から初期タスク分解を自動実行します。
...
- `execution.max_cycles`: run 全体の最大サイクル数
- `execution.phase_timeout_seconds`: 各 phase のタイムアウト（秒）
- `execution.max_consecutive_verify_failures`: verify 連続失敗で停止するしきい値
- `execution.human_escalation`: 連続失敗時/仕様曖昧時の人間委譲条件
- `execution.acceptance_criteria`: 完了判定の基準
- `execution.verification.commands`: `verify` フェーズで実行するコマンド

`execution.verification.commands` が空、またはテンプレート用のプレースホルダのままの場合、`run` 開始時点で完走しにくい構成として警告または停止対象になります。
`execution.human_escalation.on_ambiguous_spec` は現状 `run --spec` の spec bootstrap 時のみ有効です。
`blocked` タスクが存在する場合、`decide` は次サイクルへ進まず停止します。
...
実行中の構造化ログは `.orcha/agentworkspace/events.jsonl` にJSON Linesで追記されます。
サイクルごとの差分進捗（`git diff --numstat`）も `status_log.md` と `events.jsonl` に記録されます。
