...
* `execution.profile` (`local_only` / `cheap_checkpoints` / `quality_gate` / `unblock_first` / `opencode_impl_no_review` / `opencode_impl_claude_review` / `opencode_impl_codex_review` / `claude_impl_opencode_review` / `codex_impl_opencode_review`)
