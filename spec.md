...
* `execution.profile` (`local_only` / `cheap_checkpoints` / `quality_gate` / `unblock_first` / `opencode_only` / `opencode_claude` / `opencode_codex`)
...
profile変更 (local_only / cheap_checkpoints / quality_gate / unblock_first / opencode_only / opencode_claude / opencode_codex)  
