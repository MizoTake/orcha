...
pub async fn execute(orch_dir: &Path) -> anyhow::Result<()> {
    ...
    let files: Vec<(std::path::PathBuf, String)> = vec![
        ...
        (orch_dir.join("team.md"), template::team_md().to_string()),
        (
    ...
    println!("Initialized .orcha/ at {}", orch_dir.display());
    println!("  Profile: {}", default_profile);
    println!("  Run ID:  {}", run_id);
    println!();
    println!("Next steps:");
    println!("  1. Edit .orcha/goal.md with your objective");
    println!("  2. Run `orcha run` to start the first cycle");
